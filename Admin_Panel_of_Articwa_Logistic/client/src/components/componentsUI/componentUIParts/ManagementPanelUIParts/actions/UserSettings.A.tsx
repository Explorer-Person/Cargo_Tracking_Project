...
import { UserContentsInputP } from "../../../../../interface/Interfaces";
...
const ExistsUserSettingsContent = (
  handleInputDataChange: (event: MyChangeEvent, userId: string) => void,
  existsUserData: Users[] | undefined
) => {
  
  const existsElement = existsUserData?.map((userData) => {
  ...
  const existsEditableElement = existsUserData?.map((userData) => {
    const userId = userData.user_id;
    return {
      id: userId,
      status: false,
...
export const useDisplayUserInput = (
  ...
  const contentState = useAppSelector((state:RootState)=>state.InputReducer.UserContents);
  ...
  return () => { 
    if(contentState.length === 0){
      existsUserContents?.forEach((userContent) => {
        return dispatch(addUserInput(userContent))
       })
    }
...
export const useIncreaseUserInput = (
  ...
  return () => {
    dispatch(addUserInput(newContent));
  };
};
...
export const useSetInputValue = (
  ...
  return (userId: string) => {
    ...
    let editableContent: UserContentsInputP;
    if (selectedContent?.status === true) {
      ...
      return dispatch(editStatusUserInput(editableContent));
    }
    editableContent = {
      element: item,
      status: true,
    ...
    dispatch(editStatusUserInput(editableContent));
  };
};
